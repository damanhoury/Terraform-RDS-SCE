...
resource "aws_security_group" "SCE-MYSQL-SG" {
  name        = "SCE-MYSQL-SG"
  description = "Allow connection to MYSQL port 3306"
  vpc_id      = aws_vpc.SCE-VPC.id
...
resource "aws_db_instance" "SCE-DEMO-MYSQL" {
  allocated_storage      = 10
  engine                 = "mysql"
  engine_version         = "8.0.27"
  instance_class         = "db.t3.large"
  identifier             = "sce-demo-mysql"
  //name                   = "mysqltest"
  username               = "root"
  ...
  vpc_security_group_ids = ["${aws_security_group.SCE-MYSQL-SG.id}"]
}


output "MYSQL_ENDPOINT" {
  value = aws_db_instance.SCE-DEMO-MYSQL.endpoint
}