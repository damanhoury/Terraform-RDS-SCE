...
resource "aws_security_group" "SCE-MARIADB-SG" {
  name        = "SCE-MARIADB-SG"
  description = "Allow connection to MariaDB port 3306"
  vpc_id      = aws_vpc.SCE-VPC.id
...
resource "aws_db_instance" "SCE-DEMO-MARIADB" {
  allocated_storage      = 10
  engine                 = "mariadb"
  engine_version         = "10.6"
  instance_class         = "db.m6g.xlarge"
  identifier             = "sce-demo-mariadb"
  username               = "root"
  password               = "ChangeMe1948"
  publicly_accessible    = true
  skip_final_snapshot    = true
  db_subnet_group_name   = aws_db_subnet_group.SCE-DB-SUBNET-GROUP.name
  vpc_security_group_ids = ["${aws_security_group.SCE-MARIADB-SG.id}"]
}


output "MARIADB_ENDPOINT" {
  value = aws_db_instance.SCE-DEMO-MARIADB.endpoint
}